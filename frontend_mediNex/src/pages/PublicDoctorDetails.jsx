                 ...
                 {doctor.ratings.map((rating, idx) => (
                   <div key={idx} className="border-b border-gray-100 pb-6 last:border-0 last:pb-0">
                     <div className="flex items-center gap-2 mb-2">
                       ...
                       <span className="text-sm font-bold text-gray-800">{rating.patientId?.name || "Anonymous Patient"}</span>
                       {rating.date && (
                         <span className="text-xs text-gray-400 ml-auto">
                           {new Date(rating.date).toLocaleDateString()} at {new Date(rating.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                         </span>
                       )}
                     </div>
