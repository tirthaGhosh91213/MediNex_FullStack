...
export const updateSessionCredentials = async (req, res) => {
  try {
    const brokerId = req.user.id;
    const { doctorId, time_slot, meeting_link, doctor_link } = req.body;

    if (!doctorId || !time_slot || !meeting_link) {
      return res.status(400).json({ success: false, message: "Doctor ID, time slot, and meeting link are required." });
    }
    ...
    const mailOptions = {
      ...
      to: doctor.email,
      subject: `Telemedicine Session Setup: ${time_slot}`,
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
          <h2 style="color: #2563eb;">Hello Dr. ${doctor.name},</h2>
          <p>Your telemedicine session link for the <strong>${time_slot}</strong> slot has been generated by your clinic manager.</p>
          <div style="margin: 20px 0; padding: 15px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
            ...
            <a href="${doctor_link || meeting_link}" style="color: #2563eb; font-weight: bold; word-break: break-all;">${doctor_link || meeting_link}</a>
          </div>
          <p>Please click the link above when you are ready to start accepting patients from your queue.</p>
          <br/>
          <p>Best regards,<br/><strong>MediNex Automated System</strong></p>
        </div>
      `,
    };
    ...
    if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
      ...
      console.warn("EMAIL_USER or EMAIL_PASS not set in .env. Email was not sent.");
    }
...
export const deleteWaitingBooking = async (req, res) => {
  try {
    const brokerId = req.user.id;
    const { bookingId } = req.params;
    ...
    if (booking.is_session_started || booking.status === "Completed") {
      return res.status(400).json({ success: false, message: "Cannot delete an active or completed booking." });
    }

    await Booking.findByIdAndDelete(bookingId);

    // Notify the patient if connected via socket
    const io = req.app.get("io");
    if (io) {
      io.to(`patient_${booking.patientId}`).emit("booking_cancelled", {
        bookingId: booking._id,
        message: "Your booking has been cancelled by the clinic."
      });
    }

    res.status(200).json({ success: true, message: "Waiting booking deleted successfully." });
  } catch (error) {
    console.error("Delete Waiting Booking Error:", error);
    res.status(500).json({ success: false, message: "Server error" });
  }
};
